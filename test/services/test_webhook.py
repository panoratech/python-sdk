...
class TestWebhook_(unittest.TestCase):
    ...
    def test_update_webhook_status(self):
        # Mock the API response
        responses.put(
            "https://api-demo.panora.dev/webhook/5335448197", json={}, status=200
        )
        # call the method to test
        test_service = Webhook("testkey")
        response = test_service.update_webhook_status("5335448197")
        self.assertEqual(response, {})
        responses.reset(),
    ...
    def test_update_webhook_status_required_fields_missing(self):
        # Mock the API response
        responses.put(
            "https://api-demo.panora.dev/webhook/5137150747", json={}, status=202
        )
    ...
    def test_update_webhook_status_error_on_non_200(self):
        # Mock the API response
        responses.put(
            "https://api-demo.panora.dev/webhook/7223511618", json={}, status=404
        )
        with self.assertRaises(ClientException):
            test_service = Webhook("testkey")
            test_service.update_webhook_status("7223511618")
        responses.reset()
