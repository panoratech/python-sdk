...
class TestPassThroughRequestDtoModel(unittest.TestCase):
    ...
    def test_pass_through_request_dto(self):
        ...
        test_model = PassThroughRequestDto(
            path="expedita", method="GET", data={"qui": 7}, headers_={"molestias": 1}
        )
        self.assertEqual(test_model.path, "expedita")
        self.assertEqual(test_model.method, "GET")
        self.assertEqual(test_model.data, {"qui": 7})
        self.assertEqual(test_model.headers_, {"molestias": 1})
